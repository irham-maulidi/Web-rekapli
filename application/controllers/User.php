...
class User extends CI_Controller {
    ...
    private static $title = "User &minus; Arsip Rekap LI | Sungram";
            ...
            $columns = array(
                ...
                array(
                    ...
                    'formatter' => function($u_id) {
                        ...
                        <a class="btn btn-danger btn-sm mb" onclick="return confirmDialog();" href="'.site_url('user/reset/'.$u_id).'" title="Reset Password"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span> Reset</a>
                        <a class="btn btn-danger btn-sm mb" onclick="return confirmDialog();" href="'.site_url('user/deletes/'.$u_id).'" title="Hapus Permanen"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>';
                    }
                ),
            );
    ...
    public function reset()
    {
        ...
        $data = [
			'u_pass' => password_hash("sungram", PASSWORD_DEFAULT, ['cost' => 10]),
			'u_updated_at' => date('Y-m-d H:i:s'),
			'u_updated_by' => $this->session->userdata['u_id'],
    ...
    private function validation_update()
    {
        ...
        return $this->form_validation->run();
         ...
         public function deletes($u_id)
    {

        $this->load->helper('notification');
        $this->m_users->deletes($u_id, 'user');
        $this->session->set_flashdata('alert', success('Data User berhasil dihapus permanen.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/user";
        redirect(site_url('user/deleted'));
