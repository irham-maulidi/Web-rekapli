...
class Major extends CI_Controller {
    ...
    private static $title = "Program Keahlian &minus; Arsip Rekap LI | Sungram";
    ...
    public function add()
    {
        ...
        if ($this->validation()) {
            ...
            if (strtolower($data['m_name']) === strtolower($this->input->post('m_name', TRUE))) {
                $this->session->set_flashdata('alert', error('Nama Kecamatan sudah ada!'));
                $data['title'] = "Tambah ".self::$title;
                $data['form_title'] = "Tambah Kecamatan";
                $data['action'] = site_url(uri_string());
                $data['content'] = 'dashboard/major-form';
                $this->load->view('dashboard/index', $data);

            } else if (strtolower($data['m_id']) === strtolower($this->input->post('m_id', TRUE))) {
                $this->session->set_flashdata('alert', error('Inisial nama kecamatan sudah ada!'));
                $data['title'] = "Tambah ".self::$title;
                $data['form_title'] = "Tambah Kecamatan";
                $data['action'] = site_url(uri_string());
                $data['content'] = 'dashboard/major-form';
                ...
                $this->session->set_flashdata('alert', success('Data Kecamatan berhasil ditambahkan.'));
                $data['title'] = "Data ".self::$title;
                $data['content'] = "dashboard/major";
                redirect('major');
            }
    ...
    public function edit()
    {
        ...
        if ($this->validation()) {
            ...
            $this->session->set_flashdata('alert', success('Data Kecamatan berhasil diperbarui.'));
            $data['title'] = "Data ".self::$title;
            $data['content'] = "dashboard/major";
            redirect('major');
    ...
    public function delete($m_id)
    {
        ...
        $this->session->set_flashdata('alert', success('Data Kecamatan berhasil dihapus.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/major";
        $this->load->view('dashboard/index', $data);
        redirect('major');
            ...
            $columns = array(
                ...
                array(
                    ...
                    'formatter' => function($m_id) {
                        ...
                        <a class="btn btn-danger btn-sm mb" onclick="return confirmDialog();" href="'.site_url('major/deletes/'.$m_id).'" title="Hapus Permanen"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>';
                    }
                ),
            );
    ...
    public function restore()
    {
        ...
        $this->session->set_flashdata('alert', success('Data Kecamatan berhasil direstore.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/major-deleted";
    ...
    private function validation()
    {
        ...
        $this->form_validation->set_rules('m_name', 'Nama Kecamatan', 'trim|required|min_length[5]|max_length[50]');
        $this->form_validation->set_rules('m_id', 'Inisial Nama Kecamatan', 'trim|required|alpha|max_length[5]');
        return $this->form_validation->run();
    }

    public function deletes($m_id)
    {

        $this->load->helper('notification');
        $this->m_majors->deletes($m_id, 'major');
        $this->session->set_flashdata('alert', success('Data kecamatan berhasil dihapus permanen.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/major-deleted";
         redirect(site_url('major/deleted'));
